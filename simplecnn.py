...
# Get list of image filenames in nih_data/images_001/images
images_folder = os.path.join(data_path, "images_001", "images")
image_files = [f for f in os.listdir(images_folder) if f.endswith('.png')]

# Filter the CSV to include only images from images_001
...
def collate_fn(batch_indices, df, images_folder, transform):
    images = []
    labels = []
...
trainloader = DataLoader(train_indices, batch_size=CONFIG["batch_size"], shuffle=True, 
                         ...
                         collate_fn=lambda x: collate_fn(x, df, images_folder, transform))
