...
# Define image transformations (consistent with CheSS/CheXNet)
...
# Load ResNet-50 and modify for 14 classes
model = resnet50(weights=None)  # No default weights; we’ll load CheSS
model.fc = nn.Linear(model.fc.in_features, 14)

# Load CheSS pre-trained weights
chess_weights_path = "./chess_weights.pth"  # Update with actual path after downloading from https://github.com/mi2rl/CheSS
try:
    checkpoint = torch.load(chess_weights_path, map_location=CONFIG["device"])
    state_dict = checkpoint.get('state_dict', checkpoint)  # Handle MoCo v2 format
    # Remove 'module.' prefix if present (due to DataParallel training)
    state_dict = {k.replace('module.', ''): v for k, v in state_dict.items()}
    # Remove classifier weights (fc) since we’re using a new layer
    state_dict = {k: v for k, v in state_dict.items() if not k.startswith('fc.')}
    model.load_state_dict(state_dict, strict=False)  # Allow missing keys (e.g., fc)
except FileNotFoundError:
    raise FileNotFoundError(f"CheSS weights not found at {chess_weights_path}. Download from https://github.com/mi2rl/CheSS")
except Exception as e:
    raise RuntimeError(f"Error loading CheSS weights: {e}")
...
def evaluate(model, testloader, criterion, device, desc="[Test]"):
    ...
    with torch.no_grad():
        ...
        for inputs, labels in progress_bar:
            ...
            outputs = model(inputs)  # ResNet outputs [batch_size, 14]
