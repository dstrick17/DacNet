...
class CheXNetDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            image = self.transform(image)  # shape: [10, 3, 224, 224]
...
def evaluate(model, loader, criterion, device, desc="[Test]"):
    ...
    with torch.no_grad():
        for inputs, labels in tqdm(loader, desc=desc):
            bs, ncrops, c, h, w = inputs.size()  # shape: [B, 10, 3, 224, 224]
            inputs = inputs.view(-1, c, h, w).to(device)  # shape: [B*10, 3, 224, 224]
            labels = labels.to(device)

            outputs = model(inputs)  # shape: [B*10, 14]
            outputs = outputs.view(bs, ncrops, -1).mean(1)  # shape: [B, 14]
